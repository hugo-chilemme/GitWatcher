...
def find_pullrequests_by_users_and_searchterm(users=[], searchterm=""):
    ...
    if r.status_code == 200:
        ...
        # We retrieve all items that are supposed to be commits
        repoListItems = soup.find_all(class_='issue-list-item')
        ...
        for item in repoListItems:
            ...
            # We verify that the element we have retrieved is indeed a commit
            href = item.find(class_='text-bold')['href']
            title = item.find(class_='markdown-title').find('a')['title']
            
            if href is not None:
                # Get Github ID and Repository by url
                repository = href.split('/')[2]
                userId = None
                textBolds = item.find_all(class_='text-bold')
                
                for textBold in textBolds:
                    if textBold.get('title') is not None:
                        userId = textBold['title']
...
def save_commit_to_database(commit):
    ...
    query = """
        INSERT INTO commits(ID, USER_ID, REPOSITORY, TITLE, DATETIME, XP, XP_BEFORE)
        VALUES(%(id)s, %(user_id)s, %(repository)s, %(title)s, %(datetime)s, %(xp)s, %(xp_before)s)
    """
